...
\textbf{Sin umbral de utilidad.} El sistema no impone ningún piso mínimo de utilidad: la selección se rige únicamente por la función objetivo. Una versión preliminar descartaba como objetivos los recursos por debajo de un umbral (boosted $<4.0$) para evitar ``relleno'' de baja utilidad, pero al validar contra el óptimo exacto se comprobó que ese umbral dejaba utilidad alcanzable sin explotar (sección~\ref{sec:resultados}); el propio objetivo de maximización ya penaliza los recursos de baja utilidad, de modo que el umbral era innecesario y subóptimo. Se eliminó.
...
exacto del problema, es decir, del \emph{knapsack con restricciones de
precedencia}:
\[
\max \sum_i u_i x_i
\quad \text{s.a.}\quad
...
Se resuelve por \textbf{branch and bound} recorriendo los recursos en orden
topológico (prerequisitos antes que dependientes). Un recurso solo puede
incluirse cuando todos sus prerequisitos ya están incluidos —garantizando que
todo estado visitado es una selección cerrada y factible— y una cota
optimista (suma de las utilidades positivas restantes) poda las ramas que no
pueden superar la mejor solución hallada. El procedimiento es
\emph{dependency-free}, exacto y muy rápido a esta escala ($\sim 3$\,ms por
presupuesto); la misma formulación se traslada directamente a un solver ILP
para escalar a instancias de 50+ recursos.

\begin{notebox}
\textbf{El óptimo exacto como herramienta de validación.} Disponer del máximo
demostrable no solo permite cuantificar la brecha de las heurísticas: también
sirvió para auditar el propio diseño del sistema. Fue al comparar contra él
que se detectó que el umbral de utilidad (sección~4.1) producía soluciones
subóptimas, lo que motivó su eliminación.
\end{notebox}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Brecha de optimalidad:} porcentaje de la utilidad óptima
        (calculada exactamente por branch and bound) que alcanza cada heurística.
...
\midrule
15h & 14h & 33.65 & \textbf{\color{alpsgreen}100\%} & 15h & 33.65 & 8 & \textbf{\color{alpsgreen}100\%} & 33.65 \\
20h & 19h & 42.70 & \textbf{\color{alpsgreen}100\%} & 20h & 42.70 & 10 & \textbf{\color{alpsgreen}100\%} & 42.70 \\
30h & 30h & 59.30 & 96.7\% & 30h & 61.35 & 12 & \textbf{\color{alpsgreen}100\%} & 61.35 \\
\bottomrule
\end{tabular}
\end{table}

Con la función objetivo sin umbral, ambas heurísticas resultan notablemente
competentes en esta instancia. Con 15h y 20h, \textbf{Greedy y Hill Climbing
alcanzan ambos el óptimo exacto} (33.65 y 42.70 respectivamente): el problema
es lo bastante pequeño para que incluso la estrategia voraz lo resuelva de
forma óptima. La diferencia aparece únicamente en el presupuesto más amplio:
con 30h el HC alcanza el óptimo (61.35) mientras el Greedy se queda en 59.30
(96.7\,\%). Es el único caso donde las decisiones irrevocables del Greedy le
impiden la combinación óptima, y donde los \emph{swap moves} del HC marcan la
diferencia: liberan horas de recursos ya seleccionados para financiar una
reorganización —la inclusión de Evaluación y validación de modelos (3h, 9.1)—
que el Greedy no puede alcanzar.

\begin{notebox}
\textbf{El óptimo exacto reencuadra la comparación.} Disponer del máximo
demostrable cambia la lectura del experimento: no se trata tanto de que ``el
HC le gane al Greedy'' como de que, \emph{a esta escala, el problema es fácil}
y ambas heurísticas son casi óptimas. El Greedy alcanza el óptimo en 2 de 3
presupuestos y el HC en los 3. La ventaja del HC es real pero marginal
(un +3.5\,\% en el único presupuesto donde difieren) y, como se discute en la
sección de limitaciones, su valor frente al Greedy es esencialmente asintótico.
\end{notebox}

\begin{notebox}
\textbf{Nota sobre la generalización de los resultados.} El dataset de
21 recursos es suficientemente pequeño para que tanto el Greedy como el HC
encuentren soluciones óptimas o near-óptimas en casi todos los casos —y para
que el propio óptimo exacto se calcule en milisegundos. Con instancias más
grandes (50–100 recursos y grafos de dependencias más densos), donde el cálculo
exacto se vuelve intratable y el Greedy se queda corto con mayor frecuencia, la
ventaja del HC sería previsiblemente más pronunciada y consistente, dado que el
espacio de búsqueda crece exponencialmente mientras que el costo de los swap
moves crece de forma polinomial. Los resultados aquí reportados deben
interpretarse como válidos para esta instancia concreta.
\end{notebox}

\subsubsection*{Rutas óptimas — Objetivo ML Engineer}

A continuación se muestran las rutas óptimas para los presupuestos de 15h y
30h, ilustrando la progresión de recursos que el sistema selecciona al
disponer de más tiempo.

\begin{table}[H]
\centering
\small
\caption{Ruta óptima — ML Engineer, 15h. Total: 15h $|$ Utilidad: 33.65}
\begin{tabular}{r l c c}
\toprule
\textbf{\#} & \textbf{Recurso} & \textbf{Horas} & \textbf{Utilidad (boost)} \\
\midrule
1 & Álgebra lineal             & 5h & 9.1  \\
2 & Python básico              & 3h & 5.0  \\
3 & NumPy y Pandas             & 3h & 10.0 \\
4 & Preprocesamiento de texto  & 2h & 7.0  \\
5 & Visualización de datos     & 2h & 2.5  \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Ruta óptima — ML Engineer, 30h. Total: 30h $|$ Utilidad: 61.35}
\begin{tabular}{r l c c}
\toprule
\textbf{\#} & \textbf{Recurso} & \textbf{Horas} & \textbf{Utilidad (boost)} \\
\midrule
1 & Cálculo diferencial                  & 4h & 9.1  \\
2 & Álgebra lineal                       & 5h & 9.1  \\
3 & Python básico                        & 3h & 5.0  \\
4 & Probabilidad y estadística           & 4h & 5.0  \\
5 & Preprocesamiento de texto            & 2h & 7.0  \\
6 & NumPy y Pandas                       & 3h & 10.0 \\
...
\textbf{Presup.} & Horas & Util. & \% ópt & Horas & Util. & Iter. & \% ópt & Util. \\
\midrule
15h & 15h & 21.90 & \textbf{\color{alpsgreen}100\%} & 15h & 21.90 & 8 & \textbf{\color{alpsgreen}100\%} & 21.90 \\
20h & 20h & 27.25 & \textbf{\color{alpsgreen}100\%} & 20h & 27.25 & 10 & \textbf{\color{alpsgreen}100\%} & 27.25 \\
30h & 30h & 36.50 & \textbf{\color{alpsgreen}100\%} & 30h & 36.50 & 12 & \textbf{\color{alpsgreen}100\%} & 36.50 \\
\bottomrule
\end{tabular}
\end{table}
...
Como consecuencia, ambos algoritmos seleccionan solo recursos fundacionales para el objetivo NLP, y \textbf{ambos alcanzan el óptimo exacto} (100\,\%) en los tres presupuestos: aquí no hay diferencia entre Greedy y HC. La estructura del problema —no la sofisticación del algoritmo— domina por completo el resultado.

A diferencia del objetivo ML Engineer, aquí la utilidad sí crece con el presupuesto: de 21.90 (15h) a 27.25 (20h) a 36.50 (30h). El sistema invierte cada hora adicional disponible en recursos fundacionales de utilidad modesta —incluyendo \emph{Lógica y matemática discreta} (2.5), que un umbral de utilidad habría descartado pero que, al haber horas libres, contribuye a la utilidad total. Lo que el presupuesto extra \emph{no} consigue es acercar los recursos objetivo de NLP, que permanecen tras cadenas de 40h o más.
...
\midrule
15h & 33.65 & 0.00 & 33.65 & 33.65 & $[33.65,\ 33.65]$ \\
20h & 42.70 & 0.00 & 42.70 & 42.70 & $[42.70,\ 42.70]$ \\
...
El óptimo exacto cierra la interpretación: ese único óptimo dominante \emph{es} el óptimo global del problema (33.65, 42.70, 61.35), por lo que el std\,=\,0 no es solo estabilidad estadística sino la consecuencia de que todas las semillas convergen al máximo demostrable. En este objetivo el Greedy también alcanza el óptimo en 15h y 20h; solo en 30h se queda en 59.30 (96.7\,\%), que es justo el presupuesto donde el HC conserva su ventaja.
...
algoritmos. El único caso con varianza positiva en todo el estudio fue NLP
Researcher a 15h ($\sigma = 0.09$), un primer indicio —tenue— de la
estocasticidad que el HC exhibiría a mayor escala.
\end{notebox}


\subsubsection*{Rutas óptimas — Objetivo NLP Researcher}

Las siguientes tablas ilustran las rutas óptimas para el objetivo NLP
Researcher. El contraste con las rutas del objetivo ML Engineer es la
evidencia más directa del fenómeno de \emph{dependency drag}.

\begin{table}[H]
\centering
\small
\caption{Ruta óptima — NLP Researcher, 15h. Total: 15h $|$ Utilidad: 21.90}
\begin{tabular}{r l c c}
\toprule
\textbf{\#} & \textbf{Recurso} & \textbf{Horas} & \textbf{Utilidad (boost)} \\
\midrule
1 & Lógica y matemática discreta & 3h & 2.5 \\
2 & Cálculo diferencial          & 4h & 5.3 \\
3 & Python básico                & 3h & 3.4 \\
4 & NumPy y Pandas               & 3h & 3.4 \\
5 & Preprocesamiento de texto    & 2h & 7.3 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Ruta óptima — NLP Researcher, 20h. Total: 20h $|$ Utilidad: 27.25}
\begin{tabular}{r l c c}
\toprule
\textbf{\#} & \textbf{Recurso} & \textbf{Horas} & \textbf{Utilidad (boost)} \\
\midrule
...
\centering
\small
\caption{Ruta óptima — NLP Researcher, 30h. Total: 30h $|$ Utilidad: 36.50}
\begin{tabular}{r l c c}
\toprule
\textbf{\#} & \textbf{Recurso} & \textbf{Horas} & \textbf{Utilidad (boost)} \\
\midrule
1 & Cálculo diferencial                  & 4h & 5.3 \\
2 & Álgebra lineal                       & 5h & 5.3 \\
3 & Python básico                        & 3h & 3.4 \\
4 & Probabilidad y estadística           & 4h & 3.9 \\
5 & Preprocesamiento de texto            & 2h & 7.3 \\
6 & NumPy y Pandas                       & 3h & 3.4 \\
...
fundacionales de baja relevancia directa (scores de 2.5 a 5.3); el único
...
Aunque la introducción de \emph{swap moves} mejoró sustancialmente la exploración del HC (de 1 a 8–12 iteraciones efectivas para ML Engineer), en instancias con alto dependency drag el espacio de búsqueda efectivo sigue siendo pequeño y ambos algoritmos convergen trivialmente al óptimo.
...
Con 21 recursos el problema es suficientemente pequeño para que tanto el Greedy como el HC encuentren el óptimo en casi todos los casos (5 de 6 configuraciones lo alcanzan con ambos algoritmos). En datasets reales de mayor tamaño (100+ recursos) se esperaría una ventaja más pronunciada y consistente del HC.

Conviene ser explícito sobre una consecuencia de este tamaño: a la escala actual el óptimo exacto se calcula en milisegundos por branch and bound, por lo que el Hill Climbing no es estrictamente necesario aquí —el solver exacto lo domina en calidad y es viable en tiempo. La justificación del HC es \emph{asintótica}: el knapsack con restricciones de precedencia es NP-difícil, de modo que el cálculo exacto deja de ser factible al crecer la instancia, mientras que el costo de los \emph{swap moves} crece de forma polinomial. Los resultados de este trabajo deben leerse, por tanto, como una \emph{validación de corrección} de las heurísticas en una instancia donde se conoce el óptimo, no como una demostración de la necesidad práctica del HC a esta escala.
...
El haber construido el óptimo exacto como referencia permite una conclusión más matizada —y más honesta— que una simple comparación entre heurísticas. A la escala de este dataset el problema resulta \emph{fácil}: el Hill Climbing alcanza el óptimo demostrable en las seis configuraciones y el Greedy en cinco de seis. La única diferencia aparece en el presupuesto más amplio del objetivo ML Engineer (30h), donde los \emph{swap moves} del HC le dan una ventaja de +3.5\,\% sobre el Greedy. La ventaja del HC es, por tanto, real pero marginal en esta instancia; su justificación frente al baseline es esencialmente asintótica y solo se manifestaría con claridad en instancias mayores donde el cálculo exacto se vuelve intratable.

Este mismo proceso de validación contra el óptimo reveló que el umbral de utilidad del diseño inicial producía soluciones subóptimas (p.\,ej., NLP a 20h: una ruta de 27.25 era alcanzable pero el umbral limitaba la búsqueda a 21.40); eliminarlo corrigió el sesgo y es un ejemplo del valor de disponer de una verdad de referencia para auditar el propio sistema.
