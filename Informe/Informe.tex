...
Los 21 recursos se scorean secuencialmente con una pausa de 0.5s entre llamadas para respetar los límites de tasa de la API gratuita de Groq. Los scores se persisten en \texttt{scores.json} como caché, evitando recálculos en corridas sucesivas.
...
El diseño experimental compara el comportamiento de ambos algoritmos bajo dos objetivos de aprendizaje cualitativamente distintos y tres niveles de presupuesto, para un total de \textbf{12 configuraciones} (2 objetivos $\times$ 3 presupuestos $\times$ 2 algoritmos).
...
Álgebra lineal           & 2.5 & 5.0  & +2.5 & 2.0 & 4.8  & +2.8 \\
Cálculo diferencial      & 8.5 & 10.0 & +1.5 & 2.5 & 5.3  & +2.8 \\
Probabilidad y estad.    & 2.5 & 5.0  & +2.5 & 0.5 & 3.4  & +2.9 \\
NumPy y Pandas           & 6.5 & 9.1  & +2.6 & 0.5 & 3.4  & +2.9 \\
...
Redes neuronales densas  & 8.0 & 10.0 & +2.0 & 2.5 & 5.3  & +2.8 \\
...
MLOps y despliegue       & 8.5 & 8.5  & ---  & 0.5 & 0.5  & ---  \\
CNN                      & 8.5 & 8.5  & ---  & 0.5 & 0.5  & ---  \\
\bottomrule
\end{tabular}
\end{table}

El dependency boost corrige efectivamente la subvaloración de recursos fundacionales. \emph{Cálculo diferencial} recibe raw 8.5 para ML Engineer pero solo 2.5 para NLP Researcher; al ser prerequisito transitivo de \emph{Transformers} (9.5), su score boosted sube a 5.3 para el objetivo NLP. El efecto es más pronunciado en \emph{NumPy y Pandas}, base de prácticamente toda la pila de ML.

La columna NLP Researcher confirma que \emph{MLOps} (8.5 para ML Engineer) cae a 0.5 para investigación NLP, demostrando que el LLM diferencia correctamente según el perfil del usuario.
...
\centering
\small
\caption{Comparación Greedy vs.\ Hill Climbing — objetivo ML Engineer.}
\begin{tabular}{c cccc cccc c}
\toprule
& \multicolumn{3}{c}{\textbf{Greedy}} & \multicolumn{4}{c}{\textbf{Hill Climbing}} & \\
\cmidrule(lr){2-4} \cmidrule(lr){5-8}
\textbf{Presup.} & Horas & Util. & Rec. & Horas & Util. & Rec. & Iter. & \textbf{Resultado} \\
\midrule
15h & 12h & 31.15 & 4 & 15h & 33.15 & 4 & 11 & \textbf{\color{alpsgreen}HC (+6\%)} \\
20h & 17h & 40.20 & 5 & 19h & 42.20 & 5 & 11 & \textbf{\color{alpsgreen}HC (+5\%)} \\
30h & 25h & 54.30 & 7 & 30h & 61.35 & 8 & 11 & \textbf{\color{alpsgreen}HC (+13\%)} \\
\bottomrule
\end{tabular}
\end{table}

El Hill Climbing supera al Greedy en los tres presupuestos evaluados. Con 15h, el HC obtiene 33.15 frente a 31.15 del Greedy (6\% de mejora) y además utiliza las 15 horas completas mientras el Greedy solo usa 12h —tres horas de presupuesto desaprovechadas. El HC identifica que incluir Álgebra lineal (5h, 9.1) junto con Cálculo y NumPy maximiza la utilidad, combinación que el Greedy no alcanza por sus decisiones irrevocables.
...
\centering
\small
\caption{Comparación Greedy vs.\ Hill Climbing — objetivo NLP Researcher.}
\begin{tabular}{c cccc cccc c}
\toprule
& \multicolumn{3}{c}{\textbf{Greedy}} & \multicolumn{4}{c}{\textbf{Hill Climbing}} & \\
\cmidrule(lr){2-4} \cmidrule(lr){5-8}
\textbf{Presup.} & Horas & Util. & Rec. & Horas & Util. & Rec. & Iter. & \textbf{Resultado} \\
\midrule
15h & 14h & 20.90 & 4 & 14h & 20.90 & 4 & 5 & \textbf{Empate} \\
20h & 14h & 20.90 & 4 & 14h & 20.90 & 4 & 5 & \textbf{Empate} \\
30h & 30h & 36.00 & 8 & 30h & 36.00 & 8 & 5 & \textbf{Empate} \\
\bottomrule
\end{tabular}
\end{table}
...
Como consecuencia, ambos algoritmos seleccionan solo recursos fundacionales de baja utilidad para el objetivo NLP. El espacio de búsqueda efectivo es tan pequeño que ambos convergen a la misma solución sin diferenciación. Las iteraciones del HC se mantienen en 5 (1 por reinicio), confirmando que el vecindario es demasiado pequeño para explorar.
...
La comparación con el Greedy es favorable al HC en los tres presupuestos: el límite inferior del IC (33.15, 42.20, 61.35) supera en todos los casos la utilidad del Greedy (31.15, 40.20, 54.30), estableciendo la superioridad estadística del HC con confianza absoluta para esta instancia del problema.
...
\centering
\small
\caption{Ruta HC — NLP Researcher, 15h y 20h (idénticas). Total: 14h $|$ Utilidad: 20.90}
\begin{tabular}{r l c c}
\toprule
\textbf{\#} & \textbf{Recurso} & \textbf{Horas} & \textbf{Utilidad (boost)} \\
\midrule
1 & Álgebra lineal             & 5h & 4.8 \\
...
\centering
\small
\caption{Ruta HC — NLP Researcher, 30h. Total: 30h $|$ Utilidad: 36.00}
\begin{tabular}{r l c c}
\toprule
\textbf{\#} & \textbf{Recurso} & \textbf{Horas} & \textbf{Utilidad (boost)} \\
\midrule
1 & Probabilidad y estadística           & 4h & 3.4 \\
2 & Python básico                        & 3h & 3.4 \\
3 & Cálculo diferencial                  & 4h & 5.3 \\
4 & Álgebra lineal                       & 5h & 4.8 \\
5 & NumPy y Pandas                       & 3h & 3.4 \\
6 & Preprocesamiento de texto            & 2h & 7.3 \\
7 & Machine learning clásico             & 6h & 3.4 \\
8 & Evaluación y validación de modelos   & 3h & 5.0 \\
\bottomrule
\end{tabular}
\end{table}
...
máximo evaluado. El sistema se ve obligado a seleccionar recursos
fundacionales con scores de 3.4 a 5.3, útiles en sentido general pero de
baja relevancia directa para el objetivo de investigación en NLP.
...
Los resultados experimentales confirman la hipótesis principal: el Hill Climbing con swap moves supera al Greedy en escenarios de presupuesto moderado-amplio (12–15\% de mejora en utilidad para el objetivo ML Engineer), mientras que ambos son equivalentes en escenarios muy restrictivos.
